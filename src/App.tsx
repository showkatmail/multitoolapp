...
import type { TabId, Note, Settings, ImageFile, Theme, NoteVersion } from './types';
...
const NotepadView: React.FC<{
    ...
    useEffect(() => {
        ...
        recognition.onresult = (event) => {
            ...
            if (finalTranscript.trim() && activeNote) {
                const newContent = (activeNote.content || '') + finalTranscript;
    ...
    }, [showNotification, isListening, activeNote, onUpdateNote]);
        ...
        <>
            ...
            <AIAssistModal 
                ...
                onApply={(newContent) => {
                    ...
                    showNotification("AI result applied to note");
                }}
            />
...
}> = ({ isOpen, onClose, noteContent, onApply }) => {
    const [action, setAction] = useState<AIAssistAction>(AIAssistAction.GRAMMAR);
    const [isLoading, setIsLoading] = useState(false);
    const [result, setResult] = useState('');
    const textContent = useMemo(() => new DOMParser().parseFromString(noteContent, "text/html").body.textContent || "", [noteContent]);

    const handleProcess = async () => {
        setIsLoading(true);
        setResult('');
        const aiResult = await runAIAssist(action, textContent);
        setResult(aiResult);
        setIsLoading(false);
    };
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="AI Assistant">
            <div className="space-y-4">
                <div>
                    <label htmlFor="ai-action" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Action</label>
                    <select
                        id="ai-action"
                        value={action}
                        onChange={(e) => setAction(e.target.value as AIAssistAction)}
                    ...
                    >
                        {Object.values(AIAssistAction).map(act => (
                            <option key={act} value={act} className="capitalize">{act}</option>
                        ))}
                    </select>
                </div>
                 <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Original Text</label>
                    <textarea value={textContent} readOnly rows={5} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600" />
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Result</label>
                    <textarea value={result} readOnly rows={5} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600" />
                </div>
                <div className="flex justify-end space-x-2">
                    <button onClick={handleProcess} disabled={isLoading} className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:bg-indigo-300 flex items-center">
                        {isLoading && <Icon path={ICONS.SPINNER} className="w-5 h-5 mr-2 animate-spin"/>}
                        {isLoading ? 'Processing...' : 'Process'}
                    </button>
                    <button onClick={() => onApply(result)} disabled={!result} className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:bg-green-300">Apply to Note</button>
                </div>
            </div>
        </Modal>
    )
...
}> = ({ isOpen, onClose, versions, onRestore }) => {
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Version History">
            <div className="space-y-4">
                {versions.length > 0 ? versions.map((version, index) => (
                    <div key={version.timestamp} className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg">
                        <div className="flex justify-between items-center mb-2">
                            <h4 className="font-medium">Version {versions.length - index}</h4>
                            <span className="text-sm text-gray-500 dark:text-gray-400">{formatDate(version.timestamp)}</span>
                        </div>
                        <div className="bg-gray-100 dark:bg-gray-900/50 p-3 rounded max-h-40 overflow-y-auto prose dark:prose-invert max-w-none text-sm" dangerouslySetInnerHTML={{ __html: version.content }} />
                        <div className="mt-2 flex justify-end">
                            <button onClick={() => onRestore(version.content)} className="px-3 py-1 text-sm bg-indigo-600 hover:bg-indigo-700 text-white rounded">Restore this version</button>
                        </div>
                    </div>
